...
// ********************************************************************
/**
 * @file   VHDRegParamCellFlux.cc
 * @brief  score cell flux similar to G4PSCellFlux class with modification of ProcessHits(...) for MaterialOfInterest and GetIndex is definied for Regular Parameterization geometry
 *
 * @date   15th Oct 2013
 ...
 */
 ...
 VHDRegParamCellFlux::VHDRegParamCellFlux(G4String name,G4int nx, G4int ny, G4int nz,G4int depth):G4VPrimitiveScorer(name,depth),HCID(-1),weighted(TRUE),fNx(nx),fNy(ny),fNz(nz)
 {
	G4cout << "Contruct VHDRegParamCellFlux!" << G4endl;
 	fNxNy = fNx*fNy;
 	DefineUnitAndCategory();
 	SetUnit("percm2");  //default unit if cm-2
 }
 
 VHDRegParamCellFlux::VHDRegParamCellFlux(G4String name, const G4String& unit,G4int depth):G4VPrimitiveScorer(name,depth),HCID(-1),weighted(TRUE)
 {
 	DefineUnitAndCategory();
 	SetUnit(unit);  //set a preferred unit to use!
 }
 
 VHDRegParamCellFlux::~VHDRegParamCellFlux()
 {
 	if(filter){  //'filter' is a private variable that is definied in G4VPrimitiveScorer
        ...
        //clear the content of fMaterialsOfInterest
...
//   	} //when doing this here, the software was attemping to delete physical volume before it's done being used!!!
	...
	G4cout << "destroying VHDRegParamCellFlux..." << G4endl;
 }
 
 G4bool VHDRegParamCellFlux::ProcessHits(G4Step* aStep,G4TouchableHistory*)
 {	
 ...
 void VHDRegParamCellFlux::Initialize(G4HCofThisEvent* HCE)
 {
 ...
 void VHDRegParamCellFlux::EndOfEvent(G4HCofThisEvent*)
 {;}
 
 
 void VHDRegParamCellFlux::clear()
 {
 	EvtMap->clear();
 }
 
 void VHDRegParamCellFlux::DrawAll()
 {;}
 
 void VHDRegParamCellFlux::PrintAll()
 {
 ...
 void VHDRegParamCellFlux::SetUnit(const G4String& unit)
 {
 	CheckAndSetUnit(unit,"Per Unit Surface");
 }
 
 void VHDRegParamCellFlux::DefineUnitAndCategory()
 {
 	//per unit surface
 ...
 G4double VHDRegParamCellFlux::ComputeVolume(G4Step* aStep, G4int idx)
 {
 	...
 	{  //for parameterised volume
 		if(idx < 0)
 		{
 			G4Exception("VHDRegParamCellFlux","VHDRegParamCellFlux::ComputeVolume",JustWarning,"Incorrect replica number");
